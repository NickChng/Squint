...
namespace SquintScript
{
    ...
    public class AsyncPatient
    {
        ...
        public List<StructureSetHeader> GetAllStructureSets()
        {
            var L = new List<StructureSetHeader>();
            return A.Execute(new Func<Patient, List<StructureSetHeader>>((p) =>
            {
                foreach (StructureSet SS in p.StructureSets)
                    L.Add(new StructureSetHeader(SS.Id, SS.UID, ""));
                return L;
            }));
